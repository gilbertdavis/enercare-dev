        ...
        $triggers.click(function(ev) {
          if (ev.detail === 1 || !ev.detail) { // so we prevent double clicking madness (for not so savy web users) !ev.detail is for when its triggered by code
            ...
            if ($ourContentVisible) {
              if(keeponeopen === 0) {
                $ourContent.slideUp(speed, function(){
                  if ($.browser.msie) {
                    oc = $ourContent.height();
                    ci = $('#content-inner').height();
                    ni = $('.node-inner').height();
                    $('#content-inner').height(ci-oc);
                    $('#content-area').height(ci-oc);
                    $('.node-inner').height(ni-oc);
                  }
                });
            ...
            // otherwise
              ...
              if (!disablecloseothers) {
                ...
                if($visibleContent.length) {
                  $triggers.removeClass(activeClass);
                  $visibleContent.slideUp(speed, function(){
                    if ($.browser.msie){
                    vc = $visibleContent.height();
                    ci = $('#content-inner').height();
                    ni = $('.node-inner').height();
                    $('#content-inner').height(ci-vc);
                    $('#content-area').height(ci-vc);
                    $('.node-inner').height(ni-vc);
                    }
                  });
                }
              }
              // and open our section
              $ourContent.slideToggle(speed, function(){
                if ($.browser.msie) {
                    oc = $ourContent.height();
                    ci = $('#content-inner').height();
                    ni = $('.node-inner').height();
